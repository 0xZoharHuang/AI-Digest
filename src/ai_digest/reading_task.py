...
你的第一性目标是替读者从看到消息走到真正理解事物。不是逐条写文章，也不是逐条证明宣传不足。
读者跨技术、产品和创业，但不熟悉每个子领域。用具体机制、实现、例子和实验帮助他形成自己的认识。
你是本批唯一研究任务；不另开 agent/subagent。全部原文已按 reading_manifest.json 分页提供。
先读 READER.md 和 reading_manifest.json。使用 `python3 reader.py PAGE_ID` 逐页取得完整原文，
...
{"status":"skip|brief|insufficient|report", "note":"具体的一两句中文判断", "sources":["实际阅读的URL"], "report_id":null}
...
研究深度由问题决定，不规定篇数或每包相同投入。论文追正文、方法、实验与最近工作；软件追关键源码、

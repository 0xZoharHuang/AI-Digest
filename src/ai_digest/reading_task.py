...
本任务已启用 Codex 内置 live 网页工具，用它搜索、打开原文和沿链接研究；不需要新服务、代理或终端联网。
搜索与普通仓库页面可能返回旧索引，不能据此写当前 API、交付保证或安全边界。
GitHub 关键主张直接用网页工具打开 raw.githubusercontent.com 上的原始文件（优先对应 tag/commit，
仅有分支版本时如实注明），或具体 release 正文；普通页面拒绝访问时也先尝试原始文件路径。
论文用同一网页工具读取 HTML/PDF 的方法与实验，不停在摘要。这里的“回源”是取得具体原文，
不是重复打开同一份搜索摘录。终端网络受限不等于网页工具无法联网，不改机器网络设置、不另搭下载器。
区分采集时版本、索引抓取时间和所读原文版本；无法取得时保留未知，不声称当前实现保证。
README 的架构宣称不自动等于实现已验证；文档与发布说明冲突时要追实现或保留冲突，不能自行拼出结论。
所有产物（包括内部 note）的数值都要保留原指标及必要分母/条件；纠正步骤占比、任务成功率、相对改善
不能互换，缩写总结也不能改变数字的含义。报告状态的 note 主要说明研究问题，不必再压缩复制报告数字。
...
related_unit_ids 从 reading_manifest 的对应 source_anchors 按实际来源复制，不能生成或凭记忆重写 ID。
纯外部背景证据不对应输入记录时可用空列表，不能随便指派一个输入 ID。
...
交付前请实际重新打开自己的每份报告及所有简讯，站在最终读者立场完成一次回源自查，再结束任务。
尤其对“只有它能”“传统方法没有”“首次”“不可绕过”这类比较/保证句主动寻找反例：能用普通文件、
Git、现有标准或既有工具做到的事，不要包装成新系统独有；区别应落到实际新增的机器可执行语义和实现。
逐项检查数字的指标/分母、对象身份、版本、因果与作者声明的界限，修正正文与短记录之间的矛盾；
多项实验须分别保留实验主体、样本和设置，不能把 A 的成功率与 B 的工具/预算写成同一个实验。
若没有证据支持强措辞，就改成准确的有限结论。JSON 和文件数量检查不能替代这次内容审读。
所有包均有有效结果、上述审读完成后结束；最终回复只给各结果数量。
"""

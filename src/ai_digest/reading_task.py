...
搜索与网页工具的 open 都可能返回旧索引；“打开官方链接”不等于实时取得该版本。
对报告的关键 API、交付保证和安全边界，实际取得对应 tag/commit 的原始文件或 release 正文再下结论；
可用终端 HTTP 请求读取公开 raw 文件，或只读获取公开仓库的指定版本，不执行其代码。
区分采集时版本、搜索索引和当前分支，不能拼接它们的能力；原始文件也需检查所对应的版本。
无法取得时引用所见资料的版本/抓取时间并保留未核实范围，不声称当前实现保证；无需为所有背景链接重复下载。

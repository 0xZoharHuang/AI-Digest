...
def snapshots_to_remove(app_root: Path, active: Path, keep: int = 3) -> list[Path]:
    root = app_root.expanduser().resolve()
    current = active.expanduser().resolve()
...
def main() -> int:
    ...
    removed = snapshots_to_remove(args.app_root, args.active, args.keep)
    for path in removed:
